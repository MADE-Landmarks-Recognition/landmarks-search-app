...
class CGDModel(nn.Module):
    def __init__(self, backbone_type, gd_config, feature_dim, out_dim):
        super().__init__()

        # Backbone Network
        self.backbone = self.__get_backbone(backbone_type)
        ...
        assert (
            feature_dim % n == 0
        ), "the feature dim should be divided by number of global descriptors"
        ...
        for i in range(n):
            ...
            self.main_modules.append(
                nn.Sequential(nn.Linear(2048, k, bias=False), L2Norm())
            )
        ...
        self.auxiliary_module = nn.Sequential(
            nn.BatchNorm1d(2048), nn.Linear(2048, out_dim, bias=True)
        )

    def __get_backbone(self, name, pretrained=True):
        if name == "resnet50":
