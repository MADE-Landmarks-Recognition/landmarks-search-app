...
class CustomCalculator(AccuracyCalculator):
    ...
    def global_average_precision_at_k(
        self, knn_labels, query_labels, knn_distances, num_k
    ):
        ...
        relevance = EQUALITY(sorted_query_labels, sorted_knn_labels)
        cumulative_correct = torch.cumsum(relevance, dim=0)
        precision = cumulative_correct / torch.arange(
            1, num_samples + 1, device=self.device
        )
        gap = (precision * relevance).sum() / num_samples
        return gap.float().item()

    def calculate_gap_at_1(self, knn_labels, query_labels, knn_distances, **kwargs):
        return self.global_average_precision_at_k(
            knn_labels, query_labels, knn_distances, 1
        )
